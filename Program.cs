...
";
            ...
            foreach (var inputLine in inputLines) {
                if (string.IsNullOrEmpty(inputLine)) {
                    var newPassportData = new PassportData();
                    var pairs = currentPassport.ToString().Split(new[] {' ', '\r'}, StringSplitOptions.None);
                    foreach (var pair in pairs.Where(x => !string.IsNullOrEmpty(x))) {
                        var kvp = pair.Split(':');
                        if (kvp.Length > 2) {
                            throw new ArgumentException("A pair of three - how can it be?");
                        }
                        newPassportData.kvp.Add(kvp[0], kvp[1]);
                    }
                    passPortDatas.Add(newPassportData);
                    currentPassport = new StringBuilder();
                }
                else {
                    currentPassport.Append(inputLine);
                    currentPassport.Append(" ");
                }
            }

            var result = passPortDatas.Count(x => x.isValid);
    ...
    public class PassportData {
        ...
        delegate bool TestRule(string s);

        public Dictionary<string, string> kvp = new Dictionary<string, string>();

        public string byr {
            get {
                if (kvp.TryGetValue("byr", out string value)) {
                    return value;
                }
                return null;
            }
        ...
        public string iyr {
            get {
                if (kvp.TryGetValue("iyr", out string value)) {
                    return value;
                }
                return null;
            }
        ...
        public string eyr {
            get {
                if (kvp.TryGetValue("eyr", out string value)) {
                    return value;
                }
                return null;
            }
        ...
        public string hgt {
            get {
                if (kvp.TryGetValue("hgt", out string value)) {
                    return value;
                }
                return null;
            }
        ...
        public string hcl {
            get {
                if (kvp.TryGetValue("hcl", out string value)) {
                    return value;
                }
                return null;
            }
        ...
        public string ecl {
            get {
                if (kvp.TryGetValue("ecl", out string value)) {
                    return value;
                }
                return null;
            }
        ...
        public string pid {
            get {
                if (kvp.TryGetValue("pid", out string value)) {
                    return value;
                }
                return null;
            }
        ...
        public string cid {
            get {
                if (kvp.TryGetValue("cid", out string value)) {
                    return value;
                }
                return null;
            }
        ...
        public bool isValid {
            get {
                ...
                TestRule testByr = delegate(string s) {
                    var amount = Convert.ToInt32(s);
                    return (amount >= 1920 && amount <= 2002);
                };
                TestRule testIyr = delegate (string s) {
                    var amount = Convert.ToInt32(s);
                    return (amount >= 2010 && amount <= 2020);
                };
                TestRule testEyr = delegate (string s) {
                    var amount = Convert.ToInt32(s);
                    return (amount >= 2020 && amount <= 2030);
                };
                TestRule testHgt = delegate(string s) {
                        var amount = int.Parse(Regex.Replace(s, "[a-z]", ""));
                        if (s.EndsWith("cm")) {
                ...
                TestRule testHcl = delegate(string s) {
                    return Regex.IsMatch(s, "^#[0-9|a-f]{6}$");
                };
                TestRule testEcl = delegate(string s) {
                    return Regex.IsMatch(s, "^amb|blu|brn|gry|grn|hzl|oth$");
                };
                TestRule testPid = delegate(string s) {
                    return Regex.IsMatch(s, "^[0-9]{9}$");
                };
                return byr != null && testByr(byr)
                    && iyr != null && testIyr(iyr)
                    && eyr != null && testEyr(eyr)
                    && hgt != null && testHgt(hgt)
                    && hcl != null && testHcl(hcl)
                    && ecl != null && testEcl(ecl)
                    && pid != null && testPid(pid);
